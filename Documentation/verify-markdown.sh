...
if ! command -v yarn &> /dev/null; then
    echo "Error: Yarn is not installed. Please install Node.js and Yarn."
    exit 1
fi

yarn dlx markdownlint-cli2 "Documentation/**/*.md"
LINT_EXIT_CODE=$?
...
yarn dlx linkinator "Documentation/**/*.md" --markdown --recurse --verbosity error --status-code "403:ok" --skip "^(https?:\\/\\/)?(localhost|127\\.0\\.0\\.1)(:\\d+)?(\\/|$)"
LINK_EXIT_CODE=$?
